...
N_BOOT = 1000
BLOCK_SIZE = 60
TC_PER_SIDE = 0.0002              # 2 bps / side (default; override via --tc-per-side)
CHECKPOINT_EVERY = 30             # write summary.csv every N assets
...
def _save_cross_asset_png(out_dir: Path, df: pd.DataFrame):
    """Save the cross-asset comparison plot (Sharpe bars + ΔSharpe bars)."""
...
def _checkpoint(out_dir: Path, rows: list, save_png: bool):
    """Write summary.csv (and optionally PNG) for crash-safety."""
    out_dir.mkdir(parents=True, exist_ok=True)
    df = _rows_to_df(rows)
    df.to_csv(out_dir / "summary.csv", index=False)
    if save_png:
        _save_cross_asset_png(out_dir, df)
    print(f"  [checkpoint] {len(df)} assets → {out_dir / 'summary.csv'}", flush=True)


def run_one_symbol(symbol: str, OUT_DIR: Path, data_source: str = "lse",
                  dump_bars: bool = False, anchored_wf: bool = False):
    print(f"\n{'='*60}\n  {symbol}\n{'='*60}")
    if data_source == "yf":
        close = yf_fetch(symbol, interval="1d", period="max")
    else:
    ...
    probs_full = np.full((N_TEST, K_HMM), np.nan)
    win_results = []

    # Anchored WF: fit HMM ONCE on the first window's train slice, then use
    # the same frozen HMM for all subsequent windows. Per-state targets also
    # frozen from the first window. This is the strictest no-lookahead test.
    anchored_hmm = None
    anchored_targets = None
    if anchored_wf and windows:
        first_eff0, first_eff1, _, _, _ = windows[0]
        X_tr_first = hmm_X_eff[first_eff0:first_eff1]
        try:
            anchored_hmm = GaussianHMM(n_components=K_HMM, covariance_type="diag",
                                        n_iter=200, random_state=SEED, tol=1e-4,
                                        implementation="log").fit(X_tr_first)
            tr_probs_first = anchored_hmm.predict_proba(X_tr_first)
            train_logret_first = log_ret_arr[first_valid + first_eff0: first_valid + first_eff1]
            train_pred_first = rf.predict(X_test.iloc[first_valid + first_eff0: first_valid + first_eff1])
            anchored_targets, _ = select_per_state_targets(tr_probs_first, train_logret_first, train_pred_first)
            print(f"  ANCHORED WF: HMM fit on first window ({first_eff1 - first_eff0} bars), frozen for {len(windows)} windows")
        except Exception as e:
            print(f"  WARNING: anchored HMM fit failed ({e}); falling back to rolling WF")
            anchored_hmm = None
            anchored_targets = None
    ...
    for w_idx, (eff0, eff1, fs, fe, n_days) in enumerate(windows):
        ...
        # Anchored: reuse frozen targets. Rolling: optimize per window.
        if anchored_targets is not None:
            best_thr = anchored_targets
        else:
        ...
        r_net = r_win - tc_win
        m = metrics(r_net, pos_window, periods_per_year=PERIODS_PER_YEAR, vol_window=ROLL, pct_within_max=0.40)
        win_results.append({
    ...
    m_bh = metrics(bh_r[covered], np.ones(covered.sum()), periods_per_year=PERIODS_PER_YEAR, vol_window=ROLL, pct_within_max=0.40)
    m_blend = metrics(blend_r[covered], blend_pos_full[covered], periods_per_year=PERIODS_PER_YEAR, vol_window=ROLL, pct_within_max=0.40)
    ...
    for yr in sorted(set(test_idx.year)):
        ...
        m_b = metrics(bh_r[mask], np.ones(mask.sum()), periods_per_year=PERIODS_PER_YEAR, vol_window=ROLL, pct_within_max=0.40)
        m_h = metrics(blend_r[mask], blend_pos_full[mask], periods_per_year=PERIODS_PER_YEAR, vol_window=ROLL, pct_within_max=0.40)
        df_yearly.append({
    ...
    # Optional per-bar dump (regime posteriors + returns + position) — used
    # by tests/regime_decomposition.py for per-regime edge attribution.
    if dump_bars:
        # All arrays are aligned to test_idx (size N_TEST), not close.index.
...
def main():
    ...
    ap.add_argument("--tc-per-side", type=float, default=None,
                    help="TC per side as decimal (e.g. 0.0002 = 2 bps). "
                         "If not given: auto-selected as 0.0010 (10 bps) for small-cap "
                         "universe (Russell 2000, from-csv), else 0.0002 (2 bps).")
    ap.add_argument("--data-source", choices=["lse", "yf"], default="lse",
                    help="Data source: 'lse' (live LSE API, no cache) or 'yf' "
                         "(yfinance cache at data/yfinance/{ticker}_1d.csv).")
    ap.add_argument("--dump-bars", action="store_true",
                    help="Per-bar CSV with regime posteriors + returns + position "
                         "(used by tests/regime_decomposition.py)")
    ap.add_argument("--anchored-wf", action="store_true",
                    help="ANCHORED walk-forward: fit HMM once on first window, "
                         "freeze per-state targets for all windows. Strictest "
                         "no-lookahead test (vs default rolling WF).")
    args = ap.parse_args()
    ...
    elif args.data_source == "yf":
        # Auto-discover all yfinance-cached tickers
        from pathlib import Path as _P
        cache = _P("data/yfinance")
        symbols = sorted(
            f.stem.replace("_1d", "") for f in cache.glob("*_1d.csv")
        )
        print(f"Discovered {len(symbols)} tickers in data/yfinance/ (--data-source yf)")
    else:
        symbols = list(dict.fromkeys(s.strip().upper() for s in args.assets.split(",") if s.strip()))
        print(f"Using default asset list ({len(symbols)} symbols)")

    # Auto-select TC: small-cap universes (Russell 2000) → 10 bps/side.
    # Hand-picked large caps → 2 bps/side.
    if args.tc_per_side is not None:
        tc_per_side = float(args.tc_per_side)
    else:
        is_small_cap = (args.from_csv and "russell" in Path(args.from_csv).name.lower())
        tc_per_side = 0.0010 if is_small_cap else 0.0002
    print(f"Using TC_PER_SIDE = {tc_per_side*1e4:.2f} bps (round-trip = {2*tc_per_side*1e4:.2f} bps)")
    # Override the module-level constant used by run_one_symbol
    global TC_PER_SIDE
    TC_PER_SIDE = tc_per_side

    # (symbols already populated from if/elif/else above; no re-assignment needed)
    if getattr(args, "limit", None):  # legacy: --limit no longer in argparse
    ...
    for i, sym in enumerate(symbols, 1):
        t0 = _time.time()
        try:
            r = run_one_symbol(sym, OUT_DIR, data_source=args.data_source,
                               dump_bars=args.dump_bars,
                               anchored_wf=args.anchored_wf)
        except Exception as e:
            print(f"  ERROR {sym}: {e}")
            r = None
        dt = _time.time() - t0
        ...
        # Single-line progress (tmux-friendly, overwrites in place)
        pct = 100 * i / max(n_total, 1)
        bar_w = 25
        filled = int(bar_w * i / max(n_total, 1))
        bar = "█" * filled + "░" * (bar_w - filled)
        line = (
            f"\r  [{bar}] {pct:5.1f}% {i}/{n_total}  "
            f"{status} {sym:<7s} "
            f"{dt:>5.1f}s  Δ={delta_s:+.3f}  "
            f"eta={_time.strftime('%H:%M:%S', _time.gmtime(eta))}  "
            f"({done} ok, {skipped} skip)"
        )
        print(line, end="", flush=True)
        if i == n_total:
            print()  # newline at end

        # Incremental checkpoint: write summary.csv every 30 assets so a kill
        # (timeout, crash, tmux detach) doesn't lose progress. Also save
        # cross_asset.png every 60.
        if rows and (i % CHECKPOINT_EVERY == 0 or i == n_total):
            _checkpoint(OUT_DIR, rows, save_png=(i % (CHECKPOINT_EVERY * 2) == 0 or i == n_total))

    if rows:
        df = _rows_to_df(rows)
        print("\n" + "=" * 60)
        print(f"FINAL EQUITIES SUMMARY ({len(df)} assets, sorted by Δ Sharpe)")
        print("=" * 60)
        ...
        for f in sorted(OUT_DIR.glob("*")):
            if f.name in ("summary.csv", "cross_asset.png"):
