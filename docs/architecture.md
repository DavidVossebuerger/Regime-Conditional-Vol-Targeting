...
flowchart TB
    ...
    subgraph INPUT["Input (daily/hourly OHLC bars)"]
        ...
        LSE["Equities: LSE API<br/>src/data_io/lse_loader.py<br/>(no caching — fresh fetch each run)"]
    end
...
| **Per-state target** | `target_k ∈ {0.2..1.0}` | once per walk-forward window | risk-appetite per regime (joint brute-force over 9³ = 729 combos) |
